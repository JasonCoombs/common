...
CreateOrderSequence::CreateOrderSequence(const std::string& accountName
   , const QString &reqId, const bs::network::Quote& quote, const std::string &payoutTx
   , const std::shared_ptr<spdlog::logger>& logger)
 : CommandSequence("CelerCreateOrderSequence", {
         { false, nullptr, &CreateOrderSequence::createOrder }
   })
 , reqId_(reqId)
 , quote_(quote)
 , payoutTx_(payoutTx)
...
bool CreateOrderSequence::FinishSequence()
{
   return true;
}

CelerMessage CreateOrderSequence::createOrder()
{
