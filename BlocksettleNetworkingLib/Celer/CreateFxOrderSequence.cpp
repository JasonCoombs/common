...
#include "CreateFxOrderSequence.h"
...
CreateFxOrderSequence::CreateFxOrderSequence(const std::string& accountName
   , const QString &reqId, const bs::network::Quote& quote
   , const std::shared_ptr<spdlog::logger>& logger)
 : CommandSequence("CelerCreateFxOrderSequence",
      {
         { false, nullptr, &CreateFxOrderSequence::createOrder }
      })
 , reqId_(reqId)
 , quote_(quote)
 , logger_(logger)
...
bool CreateFxOrderSequence::FinishSequence()
{
   return true;
}

CelerMessage CreateFxOrderSequence::createOrder()
{
