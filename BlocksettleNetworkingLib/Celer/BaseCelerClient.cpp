...
#include "SetUserPropertySequence.h"
...
BaseCelerClient::BaseCelerClient(const std::shared_ptr<spdlog::logger> &logger
   ...
   , userId_(UserIdPropertyName)
   , submittedAuthAddressListProperty_(SubmittedBtcAuthAddressListPropertyName)
   , submittedCCAddressListProperty_(SubmittedCCAddressListPropertyName)
...
bool BaseCelerClient::SendLogin(const std::string& login, const std::string& email, const std::string& password)
{
   ...
   auto loginSequence = std::make_shared<LoginSequence>(logger_, login, password);
   ...
   , std::chrono::seconds heartbeatInterval)
{
   ...
   if (userIdRequired_) {
      auto getUserIdSequence = std::make_shared<LoadUserInfoSequence>(logger_, userName, [this]
         (bs::celer::Properties properties)
      {
         userId_ = properties[UserIdPropertyName];
         bitcoinParticipant_ = properties[BitcoinParticipantPropertyName];

         const auto authIt = properties.find(SubmittedBtcAuthAddressListPropertyName);
         ...
         const auto ccIt = properties.find(SubmittedCCAddressListPropertyName);
         if (ccIt != properties.end()) {
            submittedCCAddressListProperty_ = properties[SubmittedCCAddressListPropertyName];
            UpdateSetFromString(submittedCCAddressListProperty_.value, submittedCCAddressSet_);
         }

         const bool bd = (properties[BitcoinDealerPropertyName].value == "true");
...
bool BaseCelerClient::SendDataToSequence(const std::string& sequenceId
   , CelerAPI::CelerMessageType messageType, const std::string& message)
{
...
bool BaseCelerClient::SetSubmittedAuthAddressSet(const std::unordered_set<std::string>& addressSet)
{
   ...
   auto command = std::make_shared<SetUserPropertySequence>(logger_, userName_
...
bool BaseCelerClient::SetCCAddressSubmitted(const std::string &address)
{
   ...
   const auto command = std::make_shared<SetUserPropertySequence>(logger_, userName_
