...
template<typename T>
std::unique_ptr<SSL_CTX, void (*)(SSL_CTX*)> SlackManager<T>::CreateSSLCtx()
{
   initOpenSSL();
...
template<typename T>
bool SlackManager<T>::sendRawNotification(const std::string& message)
{
...
template<typename T>
void SlackManager<T>::processPacket(const std::string& data)
{
#ifdef WIN32
   SOCKET s;
#else
